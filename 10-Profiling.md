...
Most of the sawteeth in the cost graph correspond to the vector resizing!
...
-   First, You must compile with the `-g` flag.
-   Run your program:
    ...
    -   For example, `CPUPROFILE=gperftool.prof ./my-executable`
-   Use `pprof` to convert your output into `cachegrind` format
    -   `pprof --callgrind ./my-executable gperftool.prof > gperftool.out`
-   `kcachegrind gperftool.out` reads `gperftool.out` and displays profiling statistics!
...
-   `kcachegrind callgrind.out.NNNN` reads profiling information and displays profiling statistics!
...
-   To make every snapshot detailed, include `--detailed-freq=1`.
-   The `massif-visualizer` tool reads `massif.out` files and prints statistics for you.
...
- [Gperftool Documentation](https://gperftools.github.io/gperftools/cpuprofile.html)
- [Gperftool Source Code](https://github.com/gperftools/gperftools)
...
- [Kcachegrind Website](https://kcachegrind.github.io/html/Home.html)
...
- [Massif-visualizer source code](https://github.com/KDE/massif-visualizer)
